...
def core_calc(month, event, event_days, core):
    ...
    if event == 1:
        try: left = total - event_days; week_core = round(total/7); print((core * 3 * left) + (core * 3 * event_days * 3) + total * 5 + week_core * 15)
...
2nd argument is event, 1 means yes and 0 means no 
